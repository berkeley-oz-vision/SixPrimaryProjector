...
def initializeCalibrationPlot(gui):
    ...
    label_style = {"color": "#999", "font-size": str(default_font.pointSize()) +
                   "pt", "font-style": str(default_font.family())}
...
def activeCurrent(gui):
    ...
    for led_number in range(1, 5):
        ...
        if gui.getValue(widget):
            # Return current limit * scale
            return gui.getValue(gui.config_model["LED" + str(led_number)]["Current Limit"]) * gui.getValue(gui.calibration_current_box) / 100
    else:
        return None
...
def updatePlot(gui, y_data):
    ...
    gui.calibration_plot_window.plot(x_data[:n_samples], y_data[offset:n_samples +
                                     offset], pen=pg.mkPen('g', width=1), connect="finite")
